...
$argv = "pack /d ""$WSAppPath"" /p ""$WSAppOutputPath\$WSAppFileName.appx"" /l"
$output = Test-Process $proc $argv
if ($output -inotlike "*succeeded*") {
    Write-Output "  ERROR: Appx creation failed!"
    Write-Output "  proc = $proc"
    Write-Output "  args = $argv"
    Write-Output ("  " + $output)
    Exit 1
}
Write-Output "  Done."
...
$argv = "-n ""$WSAppPublisher"" -r -a sha256 -len 2048 -cy end -h 0 -eku 1.3.6.1.5.5.7.3.3 -b 01/01/2000 -sv ""$WSAppOutputPath\$WSAppFileName.pvk"" ""$WSAppOutputPath\$WSAppFileName.cer"""
$output = Test-Process $proc $argv
if ($output -inotlike "*succeeded*") {
...
$argv = "-pvk ""$WSAppOutputPath\$WSAppFileName.pvk"" -spc ""$WSAppOutputPath\$WSAppFileName.cer"" -pfx ""$WSAppOutputPath\$WSAppFileName.pfx"""
$output = Test-Process $proc $argv
...
$argv = "sign -fd SHA256 -a -f ""$WSAppOutputPath\$WSAppFileName.pfx"" ""$WSAppOutputPath\$WSAppFileName.appx"""
$output = Test-Process $proc $argv
