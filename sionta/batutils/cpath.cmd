...
call :trim_char "AddPath" && call :trim_char "NewPath"
if %errorlevel% equ 0 (
    endlocal & (
        if "%~1"=="/a" (
...
call :sorted -f "%temp%\show.txt" >"%temp%\show2.txt"
...
for %%A in (
    ...
    "HKCU\Environment"
) do for /f "skip=2 tokens=2*" %%B in ('reg query "%%~A" /v Path') do (
    if not defined regPath (call set "regPath=%%~C;") else (
        call set "regPath=%%regPath%%%%~C;"
    )
)
call set "regPath=%%regPath:;;=;%%" && call :trim_char regPath
if %errorlevel% equ 0 (
